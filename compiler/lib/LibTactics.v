...
(** [eq'] is an ablias for [eq] to be used for equalities in
...
(** Induction on the height of a derivation: the helper tactic
    [induct_height] helps proving the equivalence of the auxibliary
    judgment that includes a counter for the maximal height
    (see LibTacticsDemos for an example) *)

Require Import Compare_dec coqutil.Z.Lia.
...
(** Tactic [abstracts tac] is like [abstract tac] except that
    ...
    check will be happy. For example, one can use [abstracts blia]
    to obtain the same behavior as [blia] but with an auxibliary
