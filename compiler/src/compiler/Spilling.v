  ...
  Section WithMap. Local Set Default Proof Using "All".
    ...
    Proof.
      ...
      - inversion H0. subst. clear H0. cbn in *.
        ...
        replace (List.map (map.get (map.putmany (map.put map.empty a v) m')) ks)
          with (List.map (map.get m') ks). 2: {
          eapply List.map_ext_in.
          intros k HI.
          rewrite map.get_putmany_dec, map.get_put_dec, map.get_empty.
          destr (map.get m' k). 1: reflexivity.
          destr (key_eqb a k). 2: reflexivity.
          subst. contradiction.
        }
...
Section Spilling.
  ...
  Definition related'(maxvar: Z)(in_arg_regs: list Z)(first_arg_reg: Z)
             ...
             (t1: Semantics.trace)(m1: mem)(l1: locals)
             (t2: Semantics.trace)(m2: mem)(l2: locals): Prop :=
    exists lStack lRegs hlArgsRegs hlArgsStack llArgRegs stackwords argvals,
      t1 = t2 /\
      (eq m1 * word_array fpval stackwords * frame)%sep m2 /\
      map.forall_keys (fun x => fp < x < 32 /\ (x < a0 \/ a7 < x)) lRegs /\
      map.forall_keys (fun x => fp < x < 32 /\ (x < a0 \/ a7 < x)) hlArgsRegs /\
      map.forall_keys (fun x => 32 <= x <= maxvar) hlArgsStack /\
      map.forall_keys (fun x => 32 <= x <= maxvar) lStack /\
      map.of_list_zip in_arg_regs argvals = Some (map.putmany hlArgsRegs hlArgsStack) /\
      map.of_list_zip (List.unfoldn (Z.add 1) (length in_arg_regs) first_arg_reg)
                      argvals = Some llArgRegs /\
      (eq lRegs * eq hlArgsRegs * eq hlArgsStack * eq lStack)%sep l1 /\
      (eq lRegs * eq hlArgsRegs * eq llArgRegs * arg_regs * ptsto fp fpval)%sep l2 /\
      (forall r, 32 <= r <= maxvar -> forall v,
            map.get (map.putmany hlArgsStack lStack) r = Some v ->
            nth_error stackwords (Z.to_nat (r - 32)) = Some v) /\
      length stackwords = Z.to_nat (maxvar - 31).
  ...
  Proof.
    ...
    match goal with H: _ |- _ => eapply map.empty_putmany in H end. fwd.
    do 3 eexists. ssplit; try reflexivity; eauto.
    intros.
    match goal with
    | H: context[nth_error] |- _ => rename H into A
    end.
    specialize (A r).
    rewrite map.get_putmany_dec in A.
  ...
  Proof.
    unfold related', related. intros. fwd.
    eexists _, _, map.empty, map.empty, _, _, nil.
    ssplit; try reflexivity; try eassumption; fwd;
      rewrite ?map.putmany_empty_l;
      eauto using (@map.forall_keys_empty _ _ locals _ Z.eqb coqutil.Decidable.Z.eqb_spec).
  Qed.
...
(*
  ...
  Proof.
    ...
    - simpl. unfold read_register, stack_loc.
      ...
      match goal with
      | H: map.of_list_zip (_ :: _) _ = Some _ |- _ =>
        eapply map.of_list_zip_cons_keys in H; rename H into MZh
      end. fwd.
      match goal with
      | H: map.of_list_zip _ (_ :: _) = Some _ |- _ =>
        cbn -[map.of_list_zip Z.add] in H;
          eapply map.of_list_zip_cons_keys' in H; rename H into MZl
      end.
      2: eauto using List.not_In_Z_seq with zarith.
      fwd.
      match goal with
      | H: (_ * _)%sep l2 |- _ => rename H into Hl2
      end.
      rewrite eq_put_to_sep in Hl2. 2: {
      ...
      rename MZhp1 into PME. move PME at bottom.
      eapply exec.seq_cps.
      destr (map.get ksvs a).
      + replace (map.putmany (map.put map.empty a v0) ksvs) with ksvs in PME. 2: {
          apply map.map_ext. intros.
          rewrite map.get_putmany_dec, map.get_put_dec, map.get_empty.
          destr (map.get ksvs k). 1: reflexivity.
          destr (a =? k); congruence.
        }
        subst ksvs.
        rewrite map.get_putmany_dec in E.
        destr (32 <=? a).
        * destr (map.get hlArgsStack a). 2: {
            match goal with
            | H: map.forall_keys _ hlArgsRegs |- _ => specialize (H _ _ E); cbv beta in H
            end.
            exfalso; blia.
          }
          fwd.
          ...
          (* establish related' with an args list that's one shorter: *)
          unfold related'.
          eexists lStack, lRegs, hlArgsRegs, (map.put hlArgsStack a v0), _, _, _.
          ssplit.
          { reflexivity. }
          { ecancel_assumption. }
          { eassumption. }
          { eassumption. }
          { eapply map.forall_keys_put. 1: eassumption. blia. }
          { eassumption. }
          { move MZhp2 at bottom.
...
eassumption. }
          ...
          { intros b A0 w B0.
            ...
            subst b.
            rewrite map.putmany_comm in B0.
            2: eapply sep_eq_to_disjoint; ecancel_assumption.
            rewrite map.get_putmany_dec in B0.
            rewrite E1 in B0. fwd.

(* we stored a v0 to stack for a, but hlArgsStack contains w *)

              Search a. w.

            eapply Hp9.
hide_ll_arg_reg_ptsto
...
eassumption.
        * eassumption.
        * destr (map.get ksvs a).
          -- replace (map.putmany (map.put map.empty a v0) ksvs)
               with ksvs in Hl1. 2: {
               apply map.map_ext. intros.
               rewrite map.get_putmany_dec, map.get_put_dec, map.get_empty.
               destr (map.get ksvs k). 1: reflexivity. destr (a =? k); congruence.
             }
             ecancel_assumption.
          -- use_sep_assumption. cancel. case TODO. (* move put from one sep clause to another *)
        *  (* use ecancel_assumption_impl to hide (start,v0) inside arg_regs *)
          case TODO.
        *  intros r A v B.
           case TODO. (* interesting
           destr (map.get ksvs a); destr (a =? r); destruct B as [B|B];
             rewrite ?map.get_put_dec in B; fwd; try congruence.
             *)
        * blia.
      + eapply exec.seq_cps.
        eapply exec.set.
        { eapply get_sep. case TODO. }
        eapply IHargs; try eassumption; try blia.
        unfold related'. eexists _, (map.put lRegs a v0), ksvs, _, _, _.
        ssplit.
        * reflexivity.
        * ecancel_assumption.
        ...
        * eassumption.
        * eassumption.
        * use_sep_assumption. cancel. case TODO.
        * case TODO.
        * intros r A v B. specialize (Hp8 r A).
          rewrite map.get_putmany_dec, map.get_put_dec in Hp8.
          destr (a =? r). 2: {
            eapply Hp8. rewrite map.get_empty. destr (map.get ksvs r); assumption.
          }
          subst. exfalso. blia.
        * blia.
    Unshelve.
...
*)
  ...
  Lemma write_register_range_correct_aux:
    forall argsTodo argsDone argvs start e t1 t2 m1 m2 l1 l2 mc2 maxvar frame post fpval,
      related' maxvar argsDone a0 frame fpval t1 m1 l1 t2 m2 l2 ->
      (List.length argsDone + List.length argsTodo <= 8)%nat ->
      a0 + Z.of_nat (List.length argsDone) = start ->
      Forall (fun x => fp < x <= maxvar /\ (x < a0 \/ a7 < x)) argsTodo ->
      map.getmany_of_list l1 argsTodo = Some argvs ->
      (forall m2' l2' mc2',
          related' maxvar (argsDone ++ argsTodo) a0 frame fpval t1 m1 l1 t2 m2' l2' ->
          post t2 m2' l2' mc2') ->
      exec e (write_register_range start argsTodo) t2 m2 l2 mc2 post.
  Proof.
    induction argsTodo; intros.
    - simpl. subst. eapply exec.skip. rewrite List.app_nil_r in *. eauto.
    - simpl. unfold write_register, stack_loc.
      ...
      destruct argvs as [|v vs]. {
        ...
        destr (List.option_all (map (map.get l1) argsTodo)); discriminate.
      }
      ...
      unfold related' in H. fwd.
      destr (32 <=? a).
      + assert (map.get (map.putmany hlArgsStack lStack) a = Some v) as Ga. {
          assert (((eq lRegs * eq hlArgsRegs) * (eq hlArgsStack * eq lStack))%sep l1) as Hl1
              by ecancel_assumption.
          unfold sep, map.split in Hl1.
          destruct Hl1 as (lRegsAndArgs & lStackAndArgs & Sl1 & Hl1A & Hl1B).
          destruct Hl1A as (lRegs' & hlArgsRegs' & (? & ?) & ? & ?).
          subst lRegsAndArgs lRegs' hlArgsRegs'.
          destruct Hl1B as (hlArgsStack' & lStack' & (? & ?) & ? & ?).
          subst lStackAndArgs hlArgsStack' lStack'.
          destruct Sl1. subst l1.
          rewrite map.get_putmany_dec in G.
          destr (map.get (map.putmany lRegs hlArgsRegs)). {
            exfalso.
            rewrite map.get_putmany_dec in E0.
            unfold map.forall_keys in *.
            destr (map.get hlArgsRegs a);
              match goal with
              | H: _, E: map.get _ _ = Some _ |- _ => specialize H with (1 := E)
              end;
              blia.
          }
          destr (map.get (map.putmany hlArgsStack lStack) a). 2: discriminate G.
          exact G.
        }
        eapply exec.load.
        * eapply get_sep. ecancel_assumption.
        * eapply load_from_word_array. 1: ecancel_assumption. 2: blia.
          ...
          1: blia. exact Ga.
        * eapply IHargsTodo with (argsDone := argsDone ++ [a]); try eassumption; try blia.
          all: rewrite ?List.app_length.
          all: change (List.length [_]) with 1%nat.
          all: rewrite <-?List.app_assoc.
          all: cbn [List.app].
          all: try blia.
          2: solve[eauto].
          (* prove related' for IH, ie with argsDone one bigger *)
          unfold related'.
          eexists (map.remove lStack a), lRegs,
                  hlArgsRegs, (map.put hlArgsStack a v),
                  (map.put llArgRegs (a0 + Z.of_nat (length argsDone)) v),
                  _, (argvals ++ [v]).
          ssplit.
          -- reflexivity.
          -- eassumption.
          ...
          -- eassumption.
          -- eapply map.forall_keys_put. 1: eassumption. blia.
          -- intros x w A. rewrite map.get_remove_dec in A. fwd. eauto.
          -- rewrite <- map.put_putmany_commute. eapply map.of_list_zip_snoc_put; eassumption.
          -- rewrite List.app_length. change (List.length [_]) with 1%nat.
             rewrite List.unfoldn_Z_seq_snoc.
             eapply map.of_list_zip_snoc_put; eassumption.
          -- move Hp8 at bottom.
             case TODO.
          -- move Hp9 at bottom.
             (* Search ((_ * _)%sep (map.put _ _ _)). *)
             case TODO.
          -- intros r A w B.
             rewrite map.get_putmany_dec, map.get_put_dec, map.get_remove_dec in B.
             destr (a =? r). 2: {
               eapply Hp10. 1: assumption.
               rewrite map.get_putmany_dec. exact B.
             }
             fwd. eauto.
          -- assumption.
      + assert (map.get (map.putmany lRegs hlArgsRegs) a = Some v) as Ga. {
          assert (((eq lRegs * eq hlArgsRegs) * (eq hlArgsStack * eq lStack))%sep l1) as Hl1
              by ecancel_assumption.
          unfold sep, map.split in Hl1.
          destruct Hl1 as (lRegsAndArgs & lStackAndArgs & Sl1 & Hl1A & Hl1B).
          ...
          destruct Sl1. subst l1.
          rewrite map.get_putmany_dec in G.
          destr (map.get (map.putmany hlArgsStack lStack) a). {
            exfalso.
            rewrite map.get_putmany_dec in E0.
            unfold map.forall_keys in *.
            destr (map.get lStack a);
              match goal with
              | H: _, E: map.get _ _ = Some _ |- _ => specialize H with (1 := E)
              end;
              blia.
        ...
        eapply exec.set.
        * assert (((eq lRegs * eq hlArgsRegs) * (eq llArgRegs * arg_regs * ptsto fp fpval))%sep l2)
                 as Hl2 by ecancel_assumption.
          unfold sep in Hl2 at 1.
          destruct Hl2 as (lRegsAndArgs & lStackAndArgs & S22 & Hl2A & Hl2B).
          eapply map.get_split_grow_r. 1: eassumption.
          unfold sep, map.split in Hl2A. destruct Hl2A as (lRegs' & lArgsRegs' & (? & ?) & ? & ?).
          subst lRegsAndArgs lRegs' lArgsRegs'.
          exact Ga.
        * eapply IHargsTodo with (argsDone := argsDone ++ [a]); try eassumption; try blia.
          all: rewrite ?List.app_length.
          all: change (List.length [_]) with 1%nat.
          ...
          (* prove related' for IH, ie with argsDone one bigger *)
          unfold related'.
          eexists lStack, (map.remove lRegs a),
                  (map.put hlArgsRegs a v), hlArgsStack,
                  (map.put llArgRegs (a0 + Z.of_nat (length argsDone)) v),
                  _, (argvals ++ [v]).
          ssplit.
          -- reflexivity.
          ...
          -- eassumption.
          -- eassumption.
          -- rewrite map.putmany_comm. 2: {
               unfold map.disjoint. intros *. intros G1 G2.
               unfold map.forall_keys in *.
               match goal with
               | H: _, E: map.get _ _ = Some _ |- _ => specialize H with (1 := E)
               end.
               rewrite map.get_put_dec in G1. destr (a =? k).
               - fwd. blia.
               - match goal with
                 | H: _, E: map.get _ _ = Some _ |- _ => specialize H with (1 := E)
                 end.
             ...
             rewrite <- map.put_putmany_commute. eapply map.of_list_zip_snoc_put.
             rewrite map.putmany_comm. 2: {
               unfold map.disjoint. intros *. intros G1 G2.
               unfold map.forall_keys in *.
               repeat match goal with
  ...
  Lemma write_register_range_correct:
    forall args argvs e t1 t2 m1 m2 l1 l2 mc2 maxvar frame post fpval,
      related maxvar frame fpval t1 m1 l1 t2 m2 l2 ->
      (List.length args <= 8)%nat ->
      Forall (fun x => fp < x <= maxvar /\ (x < a0 \/ a7 < x)) args ->
      map.getmany_of_list l1 args = Some argvs ->
      (forall m2' l2' mc2',
          related' maxvar args a0 frame fpval t1 m1 l1 t2 m2' l2' ->
          post t2 m2' l2' mc2') ->
      exec e (write_register_range a0 args) t2 m2 l2 mc2 post.
  Proof.
    intros. eapply write_register_range_correct_aux with (argsDone := []).
    1: eapply related_to_related'_nil; eassumption.
    all: change (List.length []) with O.
    all: try blia.
    all: try eassumption.
  Qed.
  ...
  Proof.
    ...
    - (* exec.interact *)
      eapply exec.seq_cps.
      eapply write_register_range_correct; try eassumption; try (unfold a0, a7; blia).
      intros *. intro R. clear m2 l2 mc2 H4.
      ...
      subst mGive'.
      (*
      unfold sep in Rp6. destruct Rp6 as (lRegs' & lStack' & S2 & ? & ?). subst lRegs' lStack'.
      ...
      *)
      edestruct (eq_sep_to_split l2' llArgRegs) as (l2Rest & S22 & SP22). 1: ecancel_assumption.

      assert (argvals0 = argvals). {
        move Rp4 at bottom. move Rp5 at bottom.
        move H1 at bottom. move H0 at bottom. move H2 at bottom.
        eapply map.getmany_of_list_to_split in H0. fwd.
        assert ((eq lRegs * eq lStack * eq hlArgRegs)%sep l) as S2 by ecancel_assumption.
        unfold sep in S2 at 1. destruct S2 as (mrest' & hlArgRegs' & S2 & ? & ?).
        subst hlArgRegs'.
        assert (map.same_domain mks hlArgRegs) as SD. {
          move Rp4 at bottom. move H0p0 at bottom.
          case TODO.
        }
        pose proof (map.split_diff SD H0p1 S2) as D. destruct D. subst mrest' mks.
        clear SD.
        move H0p0 at bottom. move Rp4 at bottom.
        case TODO.
      }
      subst argvals0.
      ...
      + eapply map.getmany_of_list_zip_grow. 1: exact S22.
        eapply map.of_list_zip_to_getmany_of_list. 2: {
          unfold reg_class.all.
          eapply List.NoDup_firstn.
          eapply List.NoDup_filter.
          eapply List.NoDup_unfoldn_Z_seq.
        }
        rewrite arg_regs_alt by assumption.
        eassumption.
      + eassumption.
      + intros.
        match goal with
        ...
        *)
        ...
        assert (List.length (List.firstn (length resvars) (reg_class.all reg_class.arg)) =
          ...
          eapply map.putmany_of_list_zip_sameLength in Pp0. rewrite <- Pp0.
        ...
        assert (List.length (List.unfoldn (Z.add 1) (length resvars) a0) =
                List.length resvals) as HL'. {
          eapply map.putmany_of_list_zip_sameLength in Pp0. rewrite <- Pp0.
          apply List.length_unfoldn.
        }
        eapply map.sameLength_putmany_of_list with (st := map.empty) in HL'.
        destruct HL' as (resM & HresM).
        change (map.of_list_zip (List.unfoldn (Z.add 1) (length resvars) a0) resvals =
                Some resM) in HresM.
        eexists. split. 1: exact ER.
        intros.
        eapply read_register_range_correct; try eassumption; try (unfold a0, a7; blia).
        2: {
