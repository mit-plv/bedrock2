...
Section Proofs.
  ...
  Lemma save_regs_correct: forall vars offset Exec R Rexec (initial: RiscvMachineL)
      ...
      addr = Zmod.add p_sp (bits.of_Z width offset) ->
      ...
      runsTo initial (fun final =>
          ...
          final.(getPc) = Zmod.add initial.(getPc) (Zmod.mul 4
                                                   (bits.of_Z width (Z.of_nat (List.length vars)))) /\
          final.(getNextPc) = Zmod.add final.(getPc) (bits.of_Z width 4) /\
  ...
  Proof.
    ...
    - simpl in *. simp. destruct oldvalues; simpl in *; [|discriminate].
      apply runsToNonDet.runsToDone. repeat split; try assumption; try solve_word_eq.
    ...
    - simpl in *. simp.
      ...
      eapply runsToNonDet.runsTo_weaken; cycle 1;
        [|eapply IHvars with (p_sp := p_sp) (offset := (offset + bytes_per_word))
             (newvalues := l) (R := (ptsto_word (Zmod.add p_sp (bits.of_Z width offset)) z * R)%sep)]. {
  ...
  Lemma load_regs_correct: forall p_sp vars offset Exec R Rexec (initial: RiscvMachineL) values,
      ...
      (Exec * word_array (Zmod.add p_sp (bits.of_Z width offset)) values * R)%sep initial.(getMem) ->
      initial.(getNextPc) = Zmod.add initial.(getPc) (bits.of_Z width 4) ->
      valid_machine initial ->
      runsTo initial (fun final =>
          map.putmany_of_list_zip vars values initial.(getRegs) = Some final.(getRegs) /\
          final.(getMem) = initial.(getMem) /\
          final.(getPc) = Zmod.add initial.(getPc) (mul (bits.of_Z width 4)
                                                   (bits.of_Z width (Z.of_nat (List.length vars)))) /\
          final.(getNextPc) = Zmod.add final.(getPc) (bits.of_Z width 4) /\
          final.(getLog) = initial.(getLog) /\
