...
Module Export SepLogPredsWithAddrLast. Section __.
  Context {width : Z} {BW : Bitwidth.Bitwidth width} {word : Word.Interface.word width} {mem : map.map word byte}.
...
Section SepLog.
  Context {width: Z} {BW : Bitwidth.Bitwidth width} {word: word.word width} {mem: map.map word byte}.
  ...
  Lemma load_of_sep_cps: forall sz addr value R m (post: word -> Prop),
      ...
      get_option (Semantics.load sz m addr) post.
  Proof.
  ...
  Lemma load_word_of_sep_cps: forall addr value R m (post: word -> Prop),
      ...
      get_option (Semantics.load Syntax.access_size.word m addr) post.
  Proof.
  ...
  Lemma store_word_of_sep_cps_two_subgoals: forall addr oldvalue newvalue R m (post: mem -> Prop),
      ...
      get_option (Semantics.store Syntax.access_size.word m addr newvalue) post.
  Proof.
