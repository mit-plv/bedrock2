...
static __attribute__((constructor)) void _br_preconditions(void) {
  assert(((void)""two's complement"", ~(br_signed_t)0 == 0u-(br_signed_t)1));
  br_word_t u = 1;
...
Fixpoint c_expr (e : expr) : string :=
  ...
  let p_conv e := match e with (* usual arithmetic conversions to br_word_t *)
  ...
  | expr.var x => c_var x
  | expr.load s ea => "_br_load" ++ c_size s++ "(" ++ c_expr ea ++ ")"
  | expr.inlinetable s t index => "({ static const uint8_t _inlinetable["++c_lit (Z.of_nat (List.length t))++"] = {"++concat ", " (List.map HexString.of_N (List.map Byte.to_N t))++"}; _br_load"++c_size s++"((br_word_t)&_inlinetable["++c_expr index++"]); })"
  | expr.op1 op e1 => c_op1 op (p_expr e1)
  | expr.op op e1 e2 =>
    match op with
    | slu | sru | srs => c_bop (p_expr e1) op (p_shamt e2)
    | add | sub | mul
    | and | or  | xor => c_bop (p_expr e1) op (p_conv e2)
...
Fixpoint c_cmd (indent : string) (c : cmd) : string :=
  match c with
  | cmd.store s ea ev
    => indent ++ "_br_store" ++ c_size s ++ "(" ++ c_expr ea ++ ", " ++ c_expr ev ++ ");" ++ LF
