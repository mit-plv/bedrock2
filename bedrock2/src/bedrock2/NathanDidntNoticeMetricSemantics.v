...
Module exec. Section WithParams.
 ...
 Local Notation metrics := MetricLog.
  ...
  | if_true: forall t m l mc e c1 c2 post v mc',
      eval_expr m l e mc = Some (v, mc') ->
      word.unsigned v <> 0 ->
      exec c1 t m l (addMetricInstructions 2
                       (addMetricLoads 2
                       (addMetricJumps 1 mc'))) post ->
      exec (cmd.cond e c1 c2) t m l mc post
  | if_false: forall e c1 c2 t m l mc post v mc',
      eval_expr m l e mc = Some (v, mc') ->
      word.unsigned v = 0 ->
      exec c2 t m l (addMetricInstructions 2
                       (addMetricLoads 2
                       (addMetricJumps 1 mc'))) post ->
      exec (cmd.cond e c1 c2) t m l mc post
  | seq: forall c1 c2 t m l mc post mid,
      exec c1 t m l mc mid ->
      (forall t' m' l' mc', mid t' m' l' mc' -> exec c2 t' m' l' mc' post) ->
      exec (cmd.seq c1 c2) t m l mc post
  | while_false: forall e c t m l mc post v mc',
      eval_expr m l e mc = Some (v, mc') ->
      word.unsigned v = 0 ->
      post t m l (addMetricInstructions 1
                    (addMetricLoads 1
  ...
  | while_true: forall e c t m l mc post v mc' mid,
      eval_expr m l e mc = Some (v, mc') ->
      word.unsigned v <> 0 ->
      exec c t m l mc' mid ->
      (forall t' m' l' mc'', mid t' m' l' mc'' -> exec (cmd.while e c) t' m' l' (addMetricInstructions 2
                                                               (addMetricLoads 2
                                                               (addMetricJumps 1 mc''))) post) ->
      exec (cmd.while e c) t m l mc post
  | call: forall binds fname arges t m l mc post params rets fbody args mc' lf mid,
      map.get e fname = Some (params, rets, fbody) ->
      evaluate_call_args_log m l arges mc = Some (args, mc') ->
      map.of_list_zip params args = Some lf ->
      exec fbody t m lf (addMetricInstructions 100 (addMetricJumps 100 (addMetricLoads 100 (addMetricStores 100 mc')))) mid ->
      (forall t' m' st1 mc'', mid t' m' st1 mc''->
  ...
  | interact: forall binds action arges args t m l mc post mKeep mGive mc' mid,
      ...
      (forall mReceive resvals, mid mReceive resvals ->
          ...
          post (cons ((mGive, action, args), (mReceive, resvals)) t) m' l' (addMetricInstructions 1
            (addMetricStores 1
            (addMetricLoads 2 mc')))) ->
...
Section WithParams.
  ...
  Implicit Types (l: locals) (m: mem) (post: trace -> mem -> list word -> metrics -> Prop).
  ...
  Lemma weaken_call: forall e fname t m args mc post1,
      ...
      forall post2, (forall t' m' rets mc', post1 t' m' rets mc' -> post2 t' m' rets mc') ->
