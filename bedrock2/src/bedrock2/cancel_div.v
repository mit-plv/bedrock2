...
Lemma cancel_div_done: forall d x x', d <> 0 -> x = d * x' -> x / d = x'.
...
Ltac2 rec cancel_div_rec(d: constr)(e: constr): constr :=
  if Constr.equal d e then constr:(cancel_div_same $d) else
  lazy_match! e with
  | Z.add ?a ?b =>
      let pfa := cancel_div_rec d a in
      let pfb := cancel_div_rec d b in
      constr:(cancel_div_add_eq $d $a $b _ _ $pfa $pfb)
  | Z.sub ?a ?b =>
      let pfa := cancel_div_rec d a in
      let pfb := cancel_div_rec d b in
      constr:(cancel_div_sub_eq $d $a $b _ _ $pfa $pfb)
  | Z.mul ?a ?b =>
      if Constr.equal a d then constr:(@eq_refl Z $e)
      else if Constr.equal b d then constr:(Z.mul_comm $a $d)
      else first_val
        [ let pfa := cancel_div_rec d a in constr:(cancel_div_mul_l_eq $d $a _ $b $pfa)
        | let pfb := cancel_div_rec d b in constr:(cancel_div_mul_r_eq $d $b _ $a $pfb) ]
  | Z.opp ?a => let pfa := cancel_div_rec d a in constr:(cancel_div_opp_eq $d $a _ $pfa)
  end.

(* Given a divisor d, a proof that `d <> 0`, and an expression e of type Z,
   returns a proof of type `e / d = q` for some quotient q if possible, or else fails. *)
Ltac2 cancel_div(d: constr)(d_nonzero_pf: constr)(e: constr): constr :=
  let e_eq_prod_pf := cancel_div_rec d e in
  constr:(cancel_div_done $d _ _ $d_nonzero_pf $e_eq_prod_pf).
