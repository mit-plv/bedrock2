...
Lemma positional_cons {T} (f : T -> Z) B h t :
...
Notation array := (Array.array scalar (word.of_Z 8)).
Notation bytearray := (Array.array ptsto (word.of_Z 1)).
...
Local Instance spec_of_recode : spec_of "recode" :=
  fnspec! "recode" (pwords ci n : word) / words R ~> CO,
    { requires t m :=
        m =* array pwords words * R /\ length words = word.unsigned n :>Z /\
        Forall (fun b => (0 <= word.unsigned b < 2^w)) words /\ 0 <= ci <= 1;
      ensures T M := exists WORDS,
        M =* array pwords WORDS * R /\ length WORDS = word.unsigned n :>Z /\
        T = t /\ (* We have WORDS, CARRY_OUT = recode(words, carry_in) *)
        positional_signed_words (2^w) WORDS + 2^(w*n)*CO = word.unsigned ci + positional_words (2^w) words /\
        Forall (fun b => (-2^w + 2 <= 2*(word.signed b) <= 2^w)) WORDS /\ 0 <= CO <= 1
    }.
...
Proof.
  ...
  split; intro cond; repeat (straightline || apply WeakestPreconditionProperties.dexpr_expr).
  {
    ...
    repeat rewrite ?word.unsigned_and_nowrap, ?word.unsigned_sru, ?word.unsigned_add, ?word.unsigned_sub, ?word.unsigned_slu; try ZnWords.
    {
      ...
      (rewrite_strat (bottomup Z.mod_small)); rewrite ?Z.shiftr_div_pow2; try ZnWords.
      {
        ...
        assert(
        (z / 2 ^ (8 * word.unsigned idx)) mod 2 ^ 8 +
        (z / 2 ^ (8 * (word.unsigned idx + 1))) mod 2 ^ 8 * 2 ^ 8
        =
  ...
  (*rewrite word.unsigned_and, word.unsigned_sru, word.unsigned_add, word.unsigned_sub, !word.unsigned_slu; try ZnWords.*)
  repeat rewrite ?word.unsigned_and_nowrap, ?word.unsigned_sru, ?word.unsigned_add, ?word.unsigned_sub, ?word.unsigned_slu; try ZnWords.
  {
    rewrite !word.unsigned_of_Z.
    cbv [word.wrap].
  ...
  }
  {
    repeat straightline; subst br; cbv [w] in *.
    {
      ...
      destruct output as [| out0 output_rest].
      {
        ...
        rewrite word.unsigned_of_Z_nowrap in H7 by ZnWords; lia.
      }
...
Lemma swrap_sub_idemp : forall x y, @word.swrap _ word ((@word.swrap _ word x) - (@word.swrap _ word y)) =
@word.swrap _ word (x - y).
Proof.
  intros.
  unfold word.swrap.
  Z.mod_equality.
...
Proof.
  ...
  refine ((Loops.tailrec
    ...
    (fun v words R t m pwords ci n => PrimitivePair.pair.mk (* precondition *)
      (v = word.unsigned n /\
      m =* array pwords words * R /\ length words = word.unsigned n :>Z /\
      Forall (fun b => (0 <= word.unsigned b < 2^w)) words /\ 0 <= ci <= 1)
    (fun           T M PWORDS (CO : word) N => T = t /\ (* postcondition *)
      exists WORDS,
      M =* array pwords WORDS * R /\ length WORDS = word.unsigned n :>Z /\
      positional_signed_words (2^w) WORDS + 2^(w*n)*CO = word.unsigned ci + positional_words (2^w) words /\
      Forall (fun b => (-2^w + 2 <= 2*(word.signed b) <= 2^w)) WORDS /\ 0 <= CO <= 1))
  ...
  }
  {
      ...
      { rewrite List.length_nil in *; lia. }
      {
        ...
        }
        {
          ...
          { lia. }
          {
            ...
            { rewrite length_cons; ZnWords. }
            {
              cbn [positional_signed_words positional_words positional map fold_right].
              cbv [x4 x].
              case Z.ltb_spec; intros; case Z.eqb_spec; intros; try ZnWords;
              revert H13; cbv[x positional_signed_words positional_words];
              ...
              apply Z.sub_move_0_r; ring_simplify.
              {
                assert (word.signed (word.sub (word.add w0 x2) (word.of_Z 32)) = w0 + x2 - 32) as ->.
                {
                  rewrite word.signed_sub, word.signed_add, word.signed_of_Z.
                  rewrite swrap_sub_idemp.
                  rewrite !word.signed_eq_swrap_unsigned.
                  inversion H9.
                  (rewrite_strat (bottomup word.swrap_inrange)); try ZnWords. (*parser bug -> parens*)
                }
                ring_simplify.
                subst n.
              ...
              {
                rewrite word.signed_eq_swrap_unsigned.
                rewrite word.swrap_inrange by ZnWords.
              ...
              constructor.
              {
                ...
                case Z.ltb_spec; intros; case Z.eqb_spec; intros; try ZnWords; inversion H9; cbv [v0 w] in *.
                {
                  assert (word.signed (word.sub (word.add w0 x2) (word.of_Z 32)) = w0 + x2 - 32) as ->.
                  {
                    ...
                    rewrite swrap_sub_idemp.
                    rewrite !word.signed_eq_swrap_unsigned.
                    rewrite !word.swrap_inrange; ZnWords.
                  }
                  ZnWords.
                }
                assert (word.signed (word.add w0 x2) = w0 + x2) as ->.
                {
    ...
    }
    {
      ...
      { assumption. }
      {
        cbn [positional_signed_words positional_words positional List.map fold_right].
