...
Lemma positional_dist (B := 2^w) h t l :
  h <> 0 -> l > 0 ->
  let words := cons h t in
    Forall (fun b => (-2^w + 2 <= 2*b <= 2^w)) words ->
    0 < 2 * positional B words < l ->
...
Proof.
  ...
  intros [x].
  inversion H1; subst.
  case (Z.eqb_spec x 0) as [->|].
  {
    pose proof f_equal (fun x => x mod B) H3; cbv beta in *.
    rewrite Z.mod_0_l in H4 by (cbv; congruence).
    replace (h - B * positional B t) with (h + (- positional B t) * B) in H4 by lia.
    rewrite Z_mod_plus_full in H4.
    unfold B, w in *.
    Z.to_euclidean_division_equations.
    lia.
  }

  epose proof positional_bounded words _ _ H1.
  inversion H1; subst.
  epose proof positional_bounded t _ _ H7.

  unfold words in *.
  rewrite positional_cons in *.
  unfold B, w in *.

  (*replace l with 0xffffffff00000000ffffffffffffffffbce6faada7179e84f3b9cac2fc632551 in * by admit.*)

  simpl length in *.

  (*replace (length t) with 52%nat in * by admit.
  repeat (set (positional _ (repeat _ _)) as z in *; cbv in z; subst z).
  set (positional _ _) as y in *.*)

  lia.
Admitted.

(*
Lemma positional_bound_diff (n : nat):
  n > 1 -> -2^(w*n) - positional (2^w) (repeat (-2^w + 2) n) <= -2^w + 2.
Proof.
Admitted.

Lemma positional_lower_bound (n : nat) :
  - 2 ^ (w * n) <= positional (2 ^ w) (repeat (- 2 ^ w + 2) n).
Proof.
  induction n.
  { simpl. lia. }

  rewrite Nat2Z.inj_succ, <-Z.add_1_r.
  rewrite Z.mul_add_distr_l, Z.mul_1_r, Z.pow_add_r by (cbv[w]; lia).

  rewrite <-Nat.add_1_r, repeat_app, <-repeat_cons.
  rewrite positional_cons.

  assert (- (2 ^ (w * Z.of_nat n) * 2 ^ w) = 2 ^ w * - 2 ^ (w * Z.of_nat n)) as -> by lia.

  epose proof positional_bound_diff n ltac:(admit).

  set (- 2 ^ (w * Z.of_nat n)) in *.
  set (positional (2 ^ w) (repeat (- 2 ^ w + 2) n)) in *.
