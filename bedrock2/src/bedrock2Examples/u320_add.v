...
Local Instance spec_of_u320_add : spec_of "u320_add" := 
    fnspec! "u320_add" (p_x p_y p_r : word) / (x y r : list word) R ~> b,
    {
        requires t m :=
            m =* array p_x x ⋆ array p_y y ⋆ array p_r r ⋆ R 
            /\ length x = 5%nat /\ length y = 5%nat /\ 
            length r = 5%nat ;
        ensures T M := T = t /\ exists (r : list word), M =* array p_x x ⋆ array p_y y ⋆ array p_r r ⋆ R /\ 
...
Definition u320_add := func! (p_x, p_y, p_r) ~> b {
    b = $0;
