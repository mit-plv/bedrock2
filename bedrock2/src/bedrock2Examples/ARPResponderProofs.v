...
Local Notation bytes := (array scalar8 (bits.of_Z _ 1)).
...
Local Instance spec_of_arp : spec_of "arp" :=
  ...
  { requires t m := (sep (array scalar8 (bits.of_Z _ 1) ethbuf packet) R) m /\
                    Zmod.unsigned len = Z.of_nat (length packet);
    ensures T M := True }.
...
Goal program_logic_goal_for_function! arp.
  ...
  rewrite bits.unsigned_of_Z in H1.
  ...
  Ltac tload :=
  lazymatch goal with |- Memory.load Syntax.access_size.one ?m (Zmod.add ?base (Zmod.of_Z _ ?i)) = Some ?v =>
  ...
  lazymatch goal with H: _ m |- _ =>
      idtac H;
    let av := rdelta a in
    let i := lazymatch av with Zmod.add ?base (Zmod.of_Z _ ?i) => i end in
    let iNat := eval cbv in (Z.to_nat i) in
    pose i;
    SeparationLogic.seprewrite_in @array_index_nat_inbounds H;
    [instantiate (1 := iNat); blia|match goal with H : _ |- _ => instantiate (1 := byte.of_Z 0) in H end];
