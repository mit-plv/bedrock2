...
Section WithParameters.
  ...
  Proof.
    ...
    refine ((LeakageLoops.tailrec
      ...
      (fun                   K T M (X Y N R : word) => leak_bool false :: newtrace x y (Z.to_nat (Zmod.unsigned n)) ++ k = K /\ t = T /\ m = M /\
      ...
      rewrite Zmod.unsigned_0, Z.lor_0_l in H6; subst x4 v.
      unfold Zmod.eqb; setoid_rewrite Zmod.unsigned_0.
      repeat straightline. split.
      { instantiate (1 := fun _ _ _ => _ :: _). simpl. reflexivity. }
      repeat straightline. split.
      all: destr (x3 =? 0);
        rewrite ?bits.unsigned_1, ?Zmod.unsigned_0 by (pose proof width_pos; lia); eauto.
