...
Require Import bedrock2Examples.metric_LAN9250.
From coqutil Require Import Z.div_mod_to_equations.
From coqutil Require Import Word.Interface Map.Interface.
From coqutil.Tactics Require Import letexists eabstract.
From bedrock2 Require Import FE310CSemantics MetricSemantics MetricWeakestPrecondition MetricProgramLogic Array Scalars.
...
Section WithParameters.
  ...
  Instance spec_of_recvEthernet : spec_of "recvEthernet" := fun functions =>
    forall p_addr (buf:list byte) R m t mc,
      ...
      length buf = 1520%nat ->
      MetricWeakestPrecondition.call functions "recvEthernet" t m [p_addr] mc (fun t' m' rets mc' =>
  ...
  Instance spec_of_lightbulb : spec_of "lightbulb_handle" := fun functions =>
    forall p_addr (buf:list byte) (len:word) R m t mc,
      ...
      MetricWeakestPrecondition.call functions "lightbulb_handle" t m [p_addr; len] mc
  ...
  Instance spec_of_lightbulb_loop : spec_of "lightbulb_loop" := fun functions =>
    forall p_addr buf R m t mc,
      (bytes p_addr buf * R) m ->
      Z.of_nat (length buf) = 1520 ->
      MetricWeakestPrecondition.call functions "lightbulb_loop" t m [p_addr] mc
        (fun t' m' rets mc' => exists v, rets = [v] /\
  ...
  Instance spec_of_lightbulb_init : spec_of "lightbulb_init" := fun functions =>
    forall m t mc,
      MetricWeakestPrecondition.call functions "lightbulb_init" t m nil mc
        (fun t' m' rets mc' => rets = [] /\ m' = m /\
          exists iol, t' = iol ++ t /\
          exists ioh, mmio_trace_abstraction_relation ioh iol /\
          BootSeq _ ioh
        ).
  ...
  Import MetricWeakestPreconditionProperties.
  ...
  Proof.
    ...
    refine (MetricLoops.tailrec_earlyout
      ...
      (fun v scratch R t m buf num_bytes_loop i read err mc => PrimitivePair.pair.mk (
      ...
      (fun T M BUF NUM_BYTES I READ ERR MC =>
