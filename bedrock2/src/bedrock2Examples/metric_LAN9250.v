...
Section WithParameters.
  ...
  Global Instance spec_of_lan9250_readword : MetricProgramLogic.spec_of "lan9250_readword" := fun functions => forall t m a mc,
    (0x0 <= Word.Interface.word.unsigned a < 0x400) ->
    MetricWeakestPrecondition.call functions "lan9250_readword" t m [a] mc (fun T M RETS MC =>
      M = m /\
  ...
  Global Instance spec_of_lan9250_writeword : MetricProgramLogic.spec_of "lan9250_writeword" := fun functions =>
    forall t m a mc v,
      (0x0 <= Word.Interface.word.unsigned a < 0x400) ->
    (((MetricWeakestPrecondition.call functions "lan9250_writeword"))) t m [a; v] mc
      (fun T M RETS MC =>
      M = m /\
      exists err, RETS = [err] /\
      exists iol, T = iol ++ t /\
  ...
  Global Instance spec_of_lan9250_mac_write : MetricProgramLogic.spec_of "lan9250_mac_write" := fun functions =>
    ...
    (((MetricWeakestPrecondition.call functions "lan9250_mac_write"))) t m [a; v] mc
      (fun T M RETS MC =>
      M = m /\
      exists err, RETS = [err] /\
      exists iol, T = iol ++ t /\
  ...
  Global Instance spec_of_lan9250_wait_for_boot : MetricProgramLogic.spec_of "lan9250_wait_for_boot" := fun functions =>
    forall t m mc,
    (((MetricWeakestPrecondition.call functions "lan9250_wait_for_boot"))) t m [] mc
      (fun T M RETS MC =>
      M = m /\
      exists err, RETS = [err] /\
      exists iol, T = iol ++ t /\
  ...
  Global Instance spec_of_lan9250_init : MetricProgramLogic.spec_of "lan9250_init" := fun functions =>
    forall t m mc,
    (((MetricWeakestPrecondition.call functions "lan9250_init"))) t m [] mc
      (fun T M RETS MC =>
      M = m /\
      exists err, RETS = [err] /\
      exists iol, T = iol ++ t /\
  ...
  Ltac t :=
    ...
    | |- MetricWeakestPrecondition.cmd _ (cmd.interact _ _ _) _ _ _ _ _ => eapply MetricWeakestPreconditionProperties.interact_nomem; [  solve[repeat t].. | ]
    end.
  ...
  Proof.
    repeat straightline.
    refine ((atleastonce ["err"; "i"; "byteorder"] (fun v T M MC ERR I BUSY =>
  ...
  Global Instance spec_of_lan9250_tx : MetricProgramLogic.spec_of "lan9250_tx" :=
    fnspec! "lan9250_tx" p l / bs R ~> err,
    { requires t m mc := word.unsigned l = length bs /\
  ...
  Proof.
    ...
    intros; eauto.
    case H3 as ([]&?&?).
    eapply MetricWeakestPreconditionProperties.Proper_call; [|eapply H]; intuition idtac; Tactics.ssplit; eauto.
    repeat intro.
    repeat match goal with
