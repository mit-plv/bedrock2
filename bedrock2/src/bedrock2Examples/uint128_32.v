...
Section WithParameters.
  ...
  Local Notation array32 := (array scalar32 4).

  Definition eval : list word -> Z :=
    List.fold_right (fun a s => Zmod.unsigned a + 2^32*s) 0.
  ...
  Proof.
    ...
    clear dependent pa; clear dependent pb. clear -H5 mem_ok .
    ...
    repeat match goal with
    | c := if _ <? _ then _ else _ |- _ =>
        let H := fresh "H" c in
        pose proof (ltu_as_carry _ _ : Zmod.unsigned c = _) as H;
        move H before c; clearbody c; move c before mem_ok
    end.
