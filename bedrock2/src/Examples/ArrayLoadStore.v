...
Proof.
  ...
  1: (etransitivity; [etransitivity|]); cycle -1; [ | | eapply Proper_sep_iff1; [|reflexivity]; eapply bytearray_index_inbounds]; try ecancel; try Omega.omega.
  ...
  seprewrite_in (@bytearray_index_merge) H2. {
    ...
    rewrite length_firstn_inbounds;
      (PreOmega.zify; rewrite Znat.Z2Nat.id; Omega.omega).
  }
  ...
  split. {
    ...
    letexists; split. {
      ...
      Time change tt with tt in *.
      rewrite List.app_length, length_cons, length_firstn_inbounds, length_skipn_inbounds.
      all : PreOmega.zify.
      1: lia.
      1: (PreOmega.zify; rewrite ?Znat.Z2Nat.id; Omega.omega).
      1: (PreOmega.zify; rewrite ?Znat.Z2Nat.id; Omega.omega).
  } 
  1: subst v2.
  exact eq_refl.
