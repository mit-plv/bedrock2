...
Proof.
  intro e. induction e; introv CEq.
  ...
  - simpl in CEq.
    ...
    inversions CEq.
    specialize (IHe1 _ im0 p1 (p2 ++ [Add (S r2) r1 r2] ++ im1) _ rs E1).
    destruct IHe1 as [fuel1 [final1 [Run1 [Pc1 Regs1]]]].
    destruct final1 as [im pc1 rs1].
    lets Ei1: (imem_readonly_run _ _ _ Run1). simpl in Ei1. subst im.
    specialize (IHe2 _ (im0 ++ p1) p2 ([Add (S r2) r1 r2] ++ im1) _ rs1 E2).
    destruct IHe2 as [fuel2 [final2 [Run2 [Pc2 Regs2]]]].
    destruct final2 as [im pc2 rs2].
    lets Ei2: (imem_readonly_run _ _ _ Run2). simpl in Ei2. subst im.
    simpl in Pc1, Pc2. subst pc1 pc2.
    simpl in Regs1, Regs2.
    ...
    + eapply run_snoc; [ eapply R12 |].
      ...
      * rewrite Regs2.
        (* TODO show that prog2 doesn't touch lower registers *)
        assert (rs1 r1 = rs2 r1) as ER by admit.
        rewrite <- ER. rewrite Regs1.
        reflexivity.
  ...
  - admit. (* same story but with Mult *)
Qed.
...
Proof.
  introv C.
  lets E: (compile_correct_aux _ _ [] prog [] _ (initialState prog).(regs) C).
  destruct E as [fuel [final [R [Pc Regs]]]].
  exists fuel final.
