  ...
  let ldiff a b =
    (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
      ...
      (fun f0 fp fn z -> if z=0 then f0 () else if z>0 then fp z else fn (-z))
        (fun _ -> a)
        (fun b0 -> (~-)
        (Coq_N.succ_pos (Coq_N.coq_lor (Coq_Pos.pred_N a0) b0)))
        (fun b0 ->
        of_N (Coq_N.ldiff (Coq_Pos.pred_N b0) (Coq_Pos.pred_N a0)))
        b)
      a
...
| X26 ->
  ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->2*p)
    1)))))
| X27 ->
  ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->2*p)
    1)))))
| X28 ->
...
| X2a ->
  ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p)
    1)))))
| X2b ->
  ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p)
    1)))))
| X2c ->
  ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p)
    1)))))
| X2d ->
  ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p)
    1)))))
| X2e ->
  ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p)
    1)))))
| X2f ->
...
| X34 ->
  ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p)
    1)))))
| X35 ->
  ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p)
    1)))))
| X36 ->
  ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p)
    1)))))
| X37 ->
...
| X38 ->
  ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p)
    1)))))
| X39 ->
  ((fun p->1+2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p)
    1)))))
| X3a ->
...
| X3b ->
  ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p)
    ((fun p->1+2*p) 1)))))
| X3c ->
  ((fun p->2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->1+2*p)
    1)))))
| X3d ->
...
module Coq_map =
 struct
  type ('key, 'value) map = { get : (__ -> 'key -> 'value option);
                              empty : __; put : (__ -> 'key -> 'value -> __);
                              remove : (__ -> 'key -> __);
                              fold : (__ -> (__ -> 'key -> 'value -> __) ->
                                     __ -> __ -> __) }
 end

module Coq_word =
 struct
  type word = { unsigned : (__ -> int); signed : (__ -> int);
                of_Z : (int -> __); add : (__ -> __ -> __);
                sub : (__ -> __ -> __); opp : (__ -> __);
                coq_or : (__ -> __ -> __); coq_and : (__ -> __ -> __);
                xor : (__ -> __ -> __); not : (__ -> __);
                ndn : (__ -> __ -> __); mul : (__ -> __ -> __);
                mulhss : (__ -> __ -> __); mulhsu : (__ -> __ -> __);
                mulhuu : (__ -> __ -> __); divu : (__ -> __ -> __);
                divs : (__ -> __ -> __); modu : (__ -> __ -> __);
                mods : (__ -> __ -> __); slu : (__ -> __ -> __);
                sru : (__ -> __ -> __); srs : (__ -> __ -> __);
                eqb : (__ -> __ -> bool); ltu : (__ -> __ -> bool);
                lts : (__ -> __ -> bool); sextend : (int -> __ -> __) }
...
let bytes_per_word width0 =
  Coq_Z.div (Coq_Z.add width0 ((fun p->1+2*p) ((fun p->1+2*p) 1)))
    ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) 1)))
...
type parameters = { width : int; word0 : Coq_word.word;
                    mem : (Coq_word.rep, byte) Coq_map.map;
                    locals : (string, Coq_word.rep) Coq_map.map;
                    env : (string, (string list * string list) * Coq_cmd.cmd)
                          Coq_map.map }
...
(** val eqb1 :
    Coq_parameters.parameters -> Coq_parameters.key -> Coq_parameters.key ->
    bool **)
...
(** val put0 :
    ...
    list -> Coq_parameters.key -> Coq_parameters.value ->
...
let rec put0 p m k v =
  ...
  | y :: m' ->
    let (k', v') = y in
    if Coq_parameters.ltb p k k'
    then (k, v) :: m
    else if Coq_parameters.ltb p k' k
         then (k', v') :: (put0 p m' k v)
         else (k, v) :: m'

(** val remove0 :
    Coq_parameters.parameters -> (Coq_parameters.key * Coq_parameters.value)
    list -> Coq_parameters.key -> (Coq_parameters.key * Coq_parameters.value)
    list **)
...
(** val value0 :
    Coq_parameters.parameters -> rep0 ->
    (Coq_parameters.key * Coq_parameters.value) list **)
...
(** val lookup :
    Coq_parameters.parameters -> (Coq_parameters.key * Coq_parameters.value)
    list -> Coq_parameters.key -> Coq_parameters.value option **)
...
let map0 p =
  ...
  { Coq_map.get = (fun m k -> lookup p (value0 p (Obj.magic m)) k);
  Coq_map.empty = (Obj.magic []); Coq_map.put = (Obj.magic wrapped_put);
  Coq_map.remove = (Obj.magic wrapped_remove); Coq_map.fold =
  (fun _ f r0 m ->
  fold_right (fun pat r -> let (k, v) = pat in f r k v) r0
...
module ExprReflection =
 struct
  type 't expr_reifier = { er_T2w : ('t -> Coq_word.rep); er_default : 
                           't; er_opfun : (__ -> 't -> 't -> 't);
  ...
  (** val er_opname :
      parameters -> 'a1 expr_reifier -> 'a1 er_op -> Coq_bopname.bopname **)
  ...
  let expr_word_reifier semantics0 =
    { er_T2w = (fun w -> w); er_default = (semantics0.word0.Coq_word.of_Z 0);
      ...
      | Coq_bopname.Coq_lts ->
        (fun x y ->
          semantics0.word0.Coq_word.of_Z
            (Coq_Z.b2z (semantics0.word0.Coq_word.lts x y)))
      | Coq_bopname.Coq_ltu ->
        (fun x y ->
          semantics0.word0.Coq_word.of_Z
            (Coq_Z.b2z (semantics0.word0.Coq_word.ltu x y)))
      | Coq_bopname.Coq_eq ->
        (fun x y ->
          semantics0.word0.Coq_word.of_Z
            (Coq_Z.b2z (semantics0.word0.Coq_word.eqb x y)))); er_opname =
      (fun op -> Obj.magic op) }
 end
...
| Coq_cmd.Coq_stackalloc (lhs, nbytes, body) ->
  Coq_cmd.Coq_stackalloc (lhs, nbytes, (noskips body))
| Coq_cmd.Coq_cond (condition, nonzero_branch, zero_branch) ->
  Coq_cmd.Coq_cond (condition, (noskips nonzero_branch),
    (noskips zero_branch))
| Coq_cmd.Coq_seq (s1, s2) ->
  let s3 = noskips s1 in
  let s4 = noskips s2 in
  if is_skip s3
  then s4
  else if is_skip s4 then s3 else Coq_cmd.Coq_seq (s3, s4)
...
module ListArray =
 struct
  ...
  (** val get :
      'a2 hasDefault -> ('a1, nat) convertible -> 'a2 t -> 'a1 -> 'a2 **)
...
(** val offset :
    Coq_expr.expr -> Coq_expr.expr -> Coq_expr.expr -> Coq_expr.expr **)

let offset base idx width0 =
  Coq_expr.Coq_op (Coq_bopname.Coq_add, base, (Coq_expr.Coq_op
    (Coq_bopname.Coq_mul, width0, idx)))

(** val convertible_word_nat :
    parameters -> (Coq_word.rep, nat) convertible **)

let convertible_word_nat s w =
  Coq_Z.to_nat (s.word0.Coq_word.unsigned w)

(** val sortedList_parameters :
    int -> Coq_word.word -> Coq_parameters.parameters **)
...
let signed0 width0 =
  let wrap_value = fun z -> Coq_Z.modulo z (Coq_Z.pow ((fun p->2*p) 1) width0)
  in
  let swrap_value = fun z ->
    Coq_Z.sub
      (wrap_value
        (Coq_Z.add z (Coq_Z.pow ((fun p->2*p) 1) (Coq_Z.sub width0 1))))
...
let word1 width0 =
  { Coq_word.unsigned = (Obj.magic unsigned0 width0); Coq_word.signed =
    (Obj.magic signed0 width0); Coq_word.of_Z = (Obj.magic wrap width0);
    Coq_word.add = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.add (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.sub = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.sub (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.opp = (fun x ->
    Obj.magic wrap width0 (Coq_Z.opp (unsigned0 width0 (Obj.magic x))));
    Coq_word.coq_or = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.coq_lor (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.coq_and = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.coq_land (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.xor = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.coq_lxor (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.not = (fun x ->
    Obj.magic wrap width0 (Coq_Z.lnot (unsigned0 width0 (Obj.magic x))));
    Coq_word.ndn = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.ldiff (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.mul = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.mul (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.mulhss = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.div
        (Coq_Z.mul (signed0 width0 (Obj.magic x))
          (signed0 width0 (Obj.magic y))) (Coq_Z.pow ((fun p->2*p) 1) width0)));
    Coq_word.mulhsu = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.div
        (Coq_Z.mul (signed0 width0 (Obj.magic x))
          (unsigned0 width0 (Obj.magic y)))
        (Coq_Z.pow ((fun p->2*p) 1) width0))); Coq_word.mulhuu = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.div
        (Coq_Z.mul (unsigned0 width0 (Obj.magic x))
          (unsigned0 width0 (Obj.magic y)))
        (Coq_Z.pow ((fun p->2*p) 1) width0))); Coq_word.divu = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.div (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.divs = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.quot (signed0 width0 (Obj.magic x))
        (signed0 width0 (Obj.magic y)))); Coq_word.modu = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.modulo (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.mods = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.rem (signed0 width0 (Obj.magic x))
        (signed0 width0 (Obj.magic y)))); Coq_word.slu = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.shiftl (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.sru = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.shiftr (unsigned0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.srs = (fun x y ->
    Obj.magic wrap width0
      (Coq_Z.shiftr (signed0 width0 (Obj.magic x))
        (unsigned0 width0 (Obj.magic y)))); Coq_word.eqb = (fun x y ->
    Coq_Z.eqb (unsigned0 width0 (Obj.magic x))
      (unsigned0 width0 (Obj.magic y))); Coq_word.ltu = (fun x y ->
    Coq_Z.ltb (unsigned0 width0 (Obj.magic x))
      (unsigned0 width0 (Obj.magic y))); Coq_word.lts = (fun x y ->
    Coq_Z.ltb (signed0 width0 (Obj.magic x)) (signed0 width0 (Obj.magic y)));
    Coq_word.sextend = (fun oldwidth z ->
    Obj.magic wrap width0
      (Coq_Z.sub
        (Coq_Z.modulo
          (Coq_Z.add (unsigned0 width0 (Obj.magic z))
            (Coq_Z.pow ((fun p->2*p) 1) (Coq_Z.sub oldwidth 1)))
          (Coq_Z.pow ((fun p->2*p) 1) oldwidth))
        (Coq_Z.pow ((fun p->2*p) 1) (Coq_Z.sub oldwidth 1)))) }
...
(** val ranged_for' :
    int -> int -> int -> (ExitToken.t -> int -> 'a1 -> __ ->
    ExitToken.t * 'a1) -> 'a1 -> int -> 'a1 **)

let rec ranged_for' from to0 step body x x0 =
  if z_lt_dec x0 to0
  then let (tok, a1) = body (ExitToken.coq_new false) x0 x __ in
       if ExitToken.get tok
       then a1
       else ranged_for' from to0 step body a1
              (Coq_Z.add x0 (Coq_Z.max 1 step))
  else x
...
(** val ranged_for_w :
    parameters -> Coq_word.rep -> Coq_word.rep -> Coq_word.rep ->
    (Coq_word.rep -> int) -> (ExitToken.t -> Coq_word.rep -> 'a1 -> __ ->
    ExitToken.t * 'a1) -> 'a1 -> 'a1 **)

let ranged_for_w semantics0 from to0 step to_Z body a0 =
  ranged_for (to_Z from) (to_Z to0) (to_Z step) (fun tok idx acc _ ->
    body tok (semantics0.word0.Coq_word.of_Z idx) acc __) a0

(** val ranged_for_u :
    parameters -> Coq_word.rep -> Coq_word.rep -> Coq_word.rep ->
    (ExitToken.t -> Coq_word.rep -> 'a1 -> __ -> ExitToken.t * 'a1) -> 'a1 ->
    'a1 **)
...
let parameters0 =
  let word2 =
    word1 ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->2*p)
      ((fun p->2*p) 1)))))
  in
  { width = ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->2*p)
  ((fun p->2*p) 1))))); word0 = word2; mem =
  (map2 ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->2*p) ((fun p->2*p)
    1))))) word2); locals = (Obj.magic map1); env = (Obj.magic map1) }
...
module FNV1A =
 ...
 struct
  ...
  let update hash data =
    nlet ((String ((Ascii (false, false, false, false, true, true, true,
      false)), EmptyString)) :: []) P.prime (fun p ->
      nlet ((String ((Ascii (false, false, false, true, false, true, true,
        ...
        (P.semantics.word0.Coq_word.xor hash data) (fun hash0 ->
        nlet ((String ((Ascii (false, false, false, true, false, true, true,
          false)), (String ((Ascii (true, false, false, false, false, true,
          ...
          (P.semantics.word0.Coq_word.mul hash0 p) (fun hash1 -> hash1)))

  (** val update_body : Coq_cmd.cmd **)

  let update_body =
    noskips (Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (false,
      false, false, false, true, true, true, false)), EmptyString)),
      (Coq_expr.Coq_literal (P.semantics.word0.Coq_word.unsigned P.prime)))),
      (Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (false, false,
      ...
      false, true, false, true, true, false)), EmptyString)))))))),
      (ExprReflection.compile P.semantics
        (ExprReflection.expr_word_reifier P.semantics) (ExprReflection.EOp
        ((Obj.magic Coq_bopname.Coq_xor), (ExprReflection.EVar (String
        ...
        true, false)), EmptyString))))))))), (ExprReflection.EVar (String
        ...
        true, true, false)), EmptyString)))))))))))))), (Coq_cmd.Coq_seq
      ((Coq_cmd.Coq_set ((String ((Ascii (false, false, false, true, false,
      ...
      (ExprReflection.compile P.semantics
        (ExprReflection.expr_word_reifier P.semantics) (ExprReflection.EOp
        ((Obj.magic Coq_bopname.Coq_mul), (ExprReflection.EVar (String
        ((Ascii (false, false, false, true, false, true, true, false)),
        (String ((Ascii (true, false, false, false, false, true, true,
        false)), (String ((Ascii (true, true, false, false, true, true, true,
        false)), (String ((Ascii (false, false, false, true, false, true,
        ...
        EmptyString)))))))),
      (fold_right (fun v c -> Coq_cmd.Coq_seq ((Coq_cmd.Coq_unset v), c))
        Coq_cmd.Coq_skip [])))))))
  ...
  let fnv1a data len =
    nlet ((String ((Ascii (false, false, false, false, true, true, true,
      ...
      nlet ((String ((Ascii (false, false, false, true, false, true, true,
        ...
        true, false)), (String ((Ascii (true, true, false, false, true, true,
        true, false)), (String ((Ascii (false, false, false, true, false,
        true, true, false)), EmptyString)))))))) :: []) P.offset (fun hash ->
        nlet ((String ((Ascii (false, true, true, false, false, true, true,
          ...
          false, true, true, false)), EmptyString)))))))) :: [])
          (P.semantics.word0.Coq_word.of_Z 0) (fun from ->
          nlet ((String ((Ascii (true, true, false, false, true, true, true,
            false)), (String ((Ascii (false, false, true, false, true, true,
            ...
            (P.semantics.word0.Coq_word.of_Z 1) (fun step ->
            nlet ((String ((Ascii (false, false, false, true, false, true,
              ...
              (ranged_for_u P.semantics from len step (fun tok idx hash0 _ ->
                nlet ((String ((Ascii (false, true, false, false, false,
                  true, true, false)), EmptyString)) :: [])
                  (ListArray.get hasDefault_byte
                    (convertible_word_nat P.semantics) data idx) (fun b ->
                  nlet ((String ((Ascii (false, false, false, true, false,
                    ...
                    (P.semantics.word0.Coq_word.mul
                      (P.semantics.word0.Coq_word.xor hash0
                        (P.semantics.word0.Coq_word.of_Z
                          (Coq_byte.unsigned b))) p) (fun hash1 -> (tok,
                    hash1)))) hash) (fun hash0 -> hash0)))))
  ...
  let fnv1a_body =
    noskips (Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (false,
      ...
      false, false, true, true, true, false)), (String ((Ascii (false, false,
      false, true, false, true, true, false)), EmptyString)))))))),
      (Coq_expr.Coq_literal
      (P.semantics.word0.Coq_word.unsigned P.offset)))), (Coq_cmd.Coq_seq
      ((Coq_cmd.Coq_set ((String ((Ascii (false, true, true, false, false,
      ...
      true, true, false)), EmptyString)))))))),
      (ExprReflection.compile P.semantics
        (ExprReflection.expr_word_reifier P.semantics) (ExprReflection.ELit
        (0, (P.semantics.word0.Coq_word.of_Z 0), (Some __)))))),
      (Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (true, true, false,
      ...
      (ExprReflection.compile P.semantics
        (ExprReflection.expr_word_reifier P.semantics) (ExprReflection.ELit
        (1, (P.semantics.word0.Coq_word.of_Z 1), (Some __)))))),
      (Coq_cmd.Coq_seq ((Coq_cmd.Coq_while ((Coq_expr.Coq_op
      (Coq_bopname.Coq_ltu, (Coq_expr.Coq_var (String ((Ascii (false, true,
      ...
      (Coq_expr.Coq_var (String ((Ascii (false, false, true, true, false,
      true, true, false)), (String ((Ascii (true, false, true, false, false,
      true, true, false)), (String ((Ascii (false, true, true, true, false,
      true, true, false)), EmptyString))))))))), (Coq_cmd.Coq_seq
      ((Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (false, true,
      false, false, false, true, true, false)), EmptyString)),
      (Coq_expr.Coq_load (Coq_access_size.Coq_one,
      (offset (Coq_expr.Coq_var (String ((Ascii (false, false, true, false,
        false, true, true, false)), (String ((Ascii (true, false, false,
        false, false, true, true, false)), (String ((Ascii (false, false,
        true, false, true, true, true, false)), (String ((Ascii (true, false,
        false, false, false, true, true, false)), EmptyString)))))))))
        (Coq_expr.Coq_var (String ((Ascii (false, true, true, false, false,
        ...
        false, true, true, false)), EmptyString)))))))))
        (Coq_expr.Coq_literal
        (Coq_Z.of_nat (bytes_per P.semantics.width Coq_access_size.Coq_one)))))))),
      (Coq_cmd.Coq_seq ((Coq_cmd.Coq_set ((String ((Ascii (false, false,
      false, true, false, true, true, false)), (String ((Ascii (true, false,
      false, false, false, true, true, false)), (String ((Ascii (true, true,
      false, false, true, true, true, false)), (String ((Ascii (false, false,
      ...
      (ExprReflection.compile P.semantics
        (ExprReflection.expr_word_reifier P.semantics) (ExprReflection.EOp
        ((Obj.magic Coq_bopname.Coq_mul), (ExprReflection.EOp
        ((Obj.magic Coq_bopname.Coq_xor), (ExprReflection.EVar (String
        ((Ascii (false, false, false, true, false, true, true, false)),
        (String ((Ascii (true, false, false, false, false, true, true,
        false)), (String ((Ascii (true, true, false, false, true, true, true,
        ...
        EmptyString))))), (ExprReflection.EVar (String ((Ascii (false, false,
        false, false, true, true, true, false)), EmptyString)))))))),
      (fold_right (fun v c -> Coq_cmd.Coq_seq ((Coq_cmd.Coq_unset v), c))
        Coq_cmd.Coq_skip ((String ((Ascii (false, true, false, false, false,
        true, true, false)), EmptyString)) :: [])))))), (Coq_cmd.Coq_set
      ...
      EmptyString)))))))), (Coq_expr.Coq_op (Coq_bopname.Coq_add,
      (Coq_expr.Coq_var (String ((Ascii (false, true, true, false, false,
      true, true, false)), (String ((Ascii (false, true, false, false, true,
      true, true, false)), (String ((Ascii (true, true, true, true, false,
      true, true, false)), (String ((Ascii (true, false, true, true, false,
      true, true, false)), EmptyString))))))))), (Coq_expr.Coq_literal
      1))))))))),
      (fold_right (fun v c -> Coq_cmd.Coq_seq ((Coq_cmd.Coq_unset v), c))
        Coq_cmd.Coq_skip [])))))))))))
 end
...
module FNV1A32_params =
 struct
  ...
  let prime =
    Obj.magic ((fun p->1+2*p) ((fun p->1+2*p) ((fun p->2*p) ((fun p->2*p)
  ...
  let offset =
    Obj.magic ((fun p->1+2*p) ((fun p->2*p) ((fun p->1+2*p) ((fun p->2*p)
