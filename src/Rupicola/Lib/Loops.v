...
Section Gallina.
  ...
  Lemma ranged_for'_termination {idx}:
    to > idx ->
    forall n, to - (idx + S n) < to - idx.
  Proof.
    intros. (* This reduces faster than a the proof generated by ‘lia’ *)
    match goal with
    | [  |- ?a < ?b ] =>
      destruct (le_lt_dec b a) as [Hle | Hgt];
        [ exfalso; abstract lia | exact Hgt ]
    end.
  Defined.
  ...
  Function ranged_for' (a0: A) idx (Hle: from <= idx) {measure (fun n => to - n) idx} : A :=
    match le_gt_dec to idx with
    | left _ => a0
    | right Hgt =>
      let '(tok, a0) := body (ExitToken.new false) idx a0 (ranged_for'_1 Hle Hgt) in
  ...
  Proof. intros; apply ranged_for'_termination; assumption. Defined.

  Definition ranged_for (a0: A) :=
    ranged_for' a0 from ltac:(auto).

  Lemma ranged_for_invariant (P: A -> Prop) a0:
    forall (H0: P a0)
      (Hbody: forall tok idx a1 Hle, P a1 -> P (snd (body tok idx a1 Hle))),
    forall idx Hle,
...
Module test.
  ...
  Time Compute (ranged_for_w (word.of_Z 0) (word.of_Z 15) (word.of_Z 3)
                          (fun t idx acc _ =>
 ...
 *)
   ...
   (* FIXME: Note the call to lia.  Without vectors or the sizedlist predicate, we need to check that the index is in bounds but we modified the array.  Using a vector type instead keeps the information in the type and the statement of put specifies that the length is preserved. Putting that info in the representation predicates has a similar effect.  Alternatively we could do an automatic induction? *)
