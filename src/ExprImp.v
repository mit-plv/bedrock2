    ...
    Section ContStep.
      ...
      Lemma guarantees_SIO (P Q:_->Prop) binds a args
            (H :
               ...
               forall retvs m' s', external (a, argvs, m, s) (retvs, m', s') -> Q (l(*FIXME*), m', s'))
      ...
      Proof with simp.
        ...
        assert (interp_cont e 2 l m (cont_of_stmt (SIO binds a args))
          ...
          as Hstep1 by (cbn in *; match goal with H:_|-_ => erewrite H; reflexivity end).
        specialize (H3 (ex_intro _ _ (ex_intro _ _ Hstep1)))...
        cbn [lift_cont] in *...

        inversion tail...
        { 
        cbv [step] in head...
        eassert (Some ?[st'] = putmany x5 x6 l) as Hretvs by admit.
        assert (interp_cont e 2 l x1 (Imp_.CSuspended (x6, x5)) = Some (?st', x1, None)) as Hstep2.
        { cbn. match goal with |- bind_Some _ <- ?x ; _ = Some _ => destruct x end... reflexivity. }
        specialize (H3 (ex_intro _ _ (ex_intro _ _ Hstep2)))...
