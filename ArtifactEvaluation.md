...
First, please note that building the Coq development takes over three hours of CPU time and requires 8GB of RAM.
...
We are now done with formal verification claims presented in the paper.
The remainder of this section walks through our FPGA synthesis flow, in particular our usage of the recently-developed open-source toolchain for Lattice FPGAs.


First, the CPU design needs to be extracted from Coq into a format that the FPGA synthesis tools accept. 
The Kami-generated hardware designs can be reproduced using the following Makefile rule:

```sh
make -C ~/bedrock2/deps/kami proc_ext
```

File `~/bedrock2/deps/kami/Kami/Ext/Ocaml/Proc.bsv` ends up containing the Bluespec code and `~/bedrock2/deps/kami/Kami/Ext/BluespecFrontEnd/verilog/build/mkTop.v` holds the Verilog code generated by the open source Bluespec compiler.
...
The hardware and software outputs of this project are combined in `~/bedrock2/processor/integration` .
...
The FPGA bitstream is generated by the following Makefile target (and intermediate steps are described in the same Makefile):

```sh
make -C ~/bedrock2/processor/integration system.bit
```

The output generated during this command should indicate successful synthesis, reporting FPGA resource usage and maximum safe clock frequency.
