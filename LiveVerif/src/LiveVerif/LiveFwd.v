...
Require Import coqutil.Word.Bitwidth coqutil.Word.Properties coqutil.Map.Interface.
...
Section WithMem.
  ...
  Proof.
    ...
    rewrite sep_emp_l in H. apply proj2 in H. rewrite Zmod.add_0_r in H. exact H.
  Qed.

  Lemma emp_True_at_addr_sepapp_r: forall p sz m addr,
      autoforward (with_mem m (@sepapp _ _ _ p (emp_at_addr True) sz addr))
                  (with_mem m (p addr)).
  Proof.
    unfold autoforward, with_mem. intros.
    unfold emp_at_addr, sepapp in H.
